...
waited for the magic of "time" to do its thing. But no cigar. So, since everything is in Terraform
anyway I nuked the whole thing and had Terraform rebuild everything and waited some more.

Again, `internal_error`... Damn.

Then I went for a run and in the first 500 meters I had three brilliant ideas. (Note to self: run
when stuck.) The first of which was checking the GEA service it was trying to connect to.
...
called `prod`, which caused Terraform to nuke almost everything again. And then I had the wonderful
experience of doing some actual coding for a bit while Google dealt with certs and all that.

When I returned after ten minutes I was greeted by a purring LB-GEA connection.
